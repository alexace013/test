describe('Authentication Tests', () => {
    ...
    it('should load the /signup page', () => {
      cy.visit('/signup');
      cy.contains('Реєстрація').should('exist');  // Check that 'Реєстрація' form is exist
    });
    ...
    it('should login a user successfully', () => {
      cy.visit('http://127.0.0.1:3000/login');
      cy.get('input[id="field-login"]').type(validUser.login);
      cy.get('input[id="field-pasw"]').type(validUser.password);
      cy.get('button[class="submit"]').submit();
    ...
    it('should not login with invalid credentials', () => {
      ...
      cy.get('input[id="field-pasw"]').type(invalidUser.password);
      cy.get('form').submit();
